...
<div id="rab-table-wrapper" class="w-full px-3 sm:px-6 lg:px-8 py-4 md:py-8 <?= $wrapperClass ?>">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        ...
        <div class="relative w-full sm:w-100">
            <input id="rab-search" type="text" placeholder="Cari pekerjaan..."
                class="w-full pl-9 pr-4 py-2 text-xs sm:text-sm border border-slate-800 rounded-lg bg-white placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-all" />
        ...
        <div class="flex items-center gap-1.5 md:gap-2 shrink-0">
            ...
            <?php else: ?>
                <!-- Desktop Buttons -->
                <div class="hidden md:flex items-center gap-1.5 md:gap-2 shrink-0">
                    <!-- Atur Urutan -->
                    <a href="<?= base_url('menu-rap/atur-urutan?id_project=' . ($idProject ?? '') . '&slug=' . ($slug ?? '')) ?>"
                        title="Atur Urutan Uraian"
                ...
                <div class="relative md:hidden shrink-0 ml-auto sm:ml-0 z-[80]">
                    ...
                    <div id="mobileActionMenu"
                        class="hidden absolute right-0 mt-2 w-52 bg-white rounded-xl shadow-xl border border-gray-100 p-2 z-[90] animate-in fade-in zoom-in duration-200">
                        <!-- Atur Urutan -->
                        <a href="<?= base_url('menu-rap/atur-urutan?id_project=' . ($idProject ?? '') . '&slug=' . ($slug ?? '')) ?>"
                            class="flex items-center gap-3 w-full px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 rounded-lg transition-colors text-left">
                            <div class="w-7 h-7 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center shrink-0">
                                <i class="fas fa-list-ol text-xs"></i>
                            </div>
                            <span>Atur Urutan Uraian</span>
                        </a>

                        <!-- Tambah Kategori -->
                        <button id="mobile-tambah-kategori-btn" type="button"
                            class="hidden flex items-center gap-3 w-full px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 rounded-lg transition-colors text-left">
                        ...
                        <!-- Format Penomoran -->
                        <button id="mobile-format-penomoran-btn" type="button"
                            class="hidden flex items-center gap-3 w-full px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 rounded-lg transition-colors text-left">
                        ...
                        <!-- Import BOQ -->
                        <button id="mobile-boq-import-btn" type="button"
                            class="flex items-center gap-3 w-full px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 rounded-lg transition-colors text-left">
                        ...
                        <!-- Kosongkan RAP -->
                        <button id="mobile-reset-rap-btn" type="button"
                            class="flex items-center gap-3 w-full px-3 py-2 text-xs font-semibold text-red-600 hover:bg-red-50 rounded-lg transition-colors text-left">
                            <div class="w-7 h-7 rounded-lg bg-red-50 text-red-600 flex items-center justify-center shrink-0">
                                <i class="fas fa-trash-alt text-xs"></i>
                            </div>
                            <span>Kosongkan RAP</span>
                        </button>
                    </div>
                </div>
            <?php endif; ?>
    ...
    ════════════════════════════════════════════════════════════════ -->
    <div id="import-rab-modal-overlay"
        class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-[95vw] h-[85vh] flex flex-col overflow-hidden">

            <!-- Modal Header -->
            <div
                class="flex items-center justify-between px-6 py-4 border-b border-table-border bg-primary text-white rounded-t-2xl shrink-0">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-sm font-bold tracking-wide">Pemetaan Data Excel (<span id="import-file-name"
                                class="font-medium text-blue-200"></span>)</h3>
                        <p class="text-[11px] text-white/70 mt-0.5">Pilih <span
                                class="bg-white/20 px-1 rounded mx-0.5 text-white underline font-medium">Dropdown Kolom
                                Sistem</span> pada masing-masing header Excel untuk mencocokkan data.</p>
                        <p class="text-[11px] text-white/70 mt-0.5">Urutan Kolom : URAIAN, VOLUME, SATUAN</p>
                    </div>
                </div>
                <button id="import-rab-modal-close" type="button"
            ...
            <div id="import-step-1" class="flex-1 overflow-auto bg-slate-50 p-4">
                <div
                    class="rounded-xl shadow-sm border border-table-border bg-white overflow-hidden h-full flex flex-col">
                    <div class="overflow-auto flex-1 pb-4">
                        <table class="table-auto min-w-max md:w-full text-left border-collapse"
                            id="import-rab-modal-table">
                            <thead class="sticky top-0 bg-slate-100 z-10 shadow-sm border-b border-table-border"
                                id="import-rab-modal-thead">
                                <tr>
                                    <th
                                        class="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-table-subtle">
                                        Memuat Struktur Tabel...</th>
                                </tr>
                            </thead>
                            <tbody id="import-rab-modal-tbody" class="text-[11px] md:text-[13px] text-table-body">
                                <tr>
                                    <td class="text-center py-20 text-table-subtle text-xs italic">Menunggu file
                                        Excel...</td>
                                </tr>
                            </tbody>
                        </table>
            ...
            <div id="import-step-2" class="hidden flex-1 overflow-hidden bg-slate-50 flex flex-col">
                <!-- Toolbar -->
                <div
                    class="px-6 py-3 bg-white border-b border-table-border flex items-center justify-between shadow-sm shrink-0">
                    <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-3 w-full justify-between">
                        <div class="flex items-center gap-2 shrink-0">
                            <button id="import-organize-indent-in" type="button" title="Jadikan Sub (Indentasi Masuk)"
                                class="p-1.5 md:px-3 md:py-2 rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition-all text-xs font-bold flex items-center gap-1.5">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <button id="import-organize-indent-out" type="button"
                                title="Naikkan Level (Indentasi Keluar)"
                                class="p-1.5 md:px-3 md:py-2 rounded-lg bg-amber-50 text-amber-600 hover:bg-amber-100 transition-all text-xs font-bold flex items-center gap-1.5">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="flex items-center gap-2 bg-slate-50 p-1 rounded-lg border border-slate-200">
                            <div class="flex items-center gap-1.5 px-2 border-r border-slate-200">
                                <input id="import-custom-kategori-input" type="text" placeholder="Tambah Kategori..."
                                    class="bg-transparent border-none text-[11px] md:text-xs focus:ring-0 p-1 w-32 md:w-40 font-medium placeholder:text-slate-400">
                                <button id="import-custom-kategori-add" type="button"
                                    class="text-emerald-500 hover:text-emerald-700 p-1 transition-colors"
                                    title="Tambah Kategori Baru">
                                    <i class="fas fa-plus-circle text-sm"></i>
                                </button>
                            </div>

                            <select id="import-global-kategori"
                                class="bg-transparent border-none text-slate-700 text-[11px] md:text-xs focus:ring-0 block p-1 md:p-1.5 font-medium min-w-[150px]">
                                <option value="">-- Pilih Kategori --</option>
                            </select>
                            <button id="import-organize-insert-cat" type="button"
                                class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded bg-primary text-white hover:bg-primary-hover text-[10px] md:text-xs font-bold transition-all whitespace-nowrap shadow-sm">
                ...
                <div class="flex-1 overflow-auto p-4 lg:p-6">
                    ...
                    <div
                        class="max-w-4xl mx-auto bg-white rounded-xl shadow-sm border border-table-border overflow-hidden">
            ...
            <!-- Modal Footer -->
            <div
                class="flex flex-col-reverse md:flex-row md:items-center md:justify-between gap-2 px-4 md:px-6 py-3 md:py-4 border-t border-table-border bg-white shrink-0 rounded-b-2xl shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-20">
                <div class="flex items-center gap-4">
                    <p id="import-rab-modal-count"
                        class="text-[10px] md:text-xs text-table-subtle font-medium text-center md:text-left">
                        Pilih file Excel untuk memulai
                    </p>
                </div>
                <div class="flex items-center justify-end gap-2">
                    <button id="import-rab-modal-back" type="button"
                        class="hidden whitespace-nowrap px-4 py-1.5 md:py-2 rounded-lg border border-table-border bg-white hover:bg-slate-50 text-table-body text-[10px] md:text-xs font-medium transition-all focus:outline-none active:scale-95">
                        <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                        </svg> Kembali
                    </button>
                    <button id="import-rab-modal-repick" type="button"
                        class="whitespace-nowrap px-4 py-1.5 md:py-2 rounded-lg border border-table-border bg-white hover:bg-slate-50 text-table-body text-[10px] md:text-xs font-medium transition-all focus:outline-none active:scale-95">
                        <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                        </svg> Ganti File Excel
                    </button>
                    <button id="import-rab-modal-cancel" type="button"
                        class="whitespace-nowrap px-4 py-1.5 md:py-2 rounded-lg border border-table-border bg-white hover:bg-slate-50 text-table-body text-[10px] md:text-xs font-medium transition-all focus:outline-none active:scale-95">
                        Batal
                    </button>
                    <button id="import-rab-modal-next" type="button"
                        class="hidden whitespace-nowrap px-4 md:px-6 py-1.5 md:py-2 rounded-lg bg-primary hover:bg-primary-hover text-white text-[10px] md:text-xs font-bold tracking-wide shadow-sm transition-all duration-150 focus:outline-none active:scale-95 disabled:opacity-50">
                        Lanjut Organisir <svg class="w-4 h-4 inline ml-1" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                        </svg>
                    </button>
                    <button id="import-rab-modal-confirm" type="button"
                        class="hidden whitespace-nowrap px-4 md:px-6 py-1.5 md:py-2 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white text-[10px] md:text-xs font-bold tracking-wide shadow-sm transition-all duration-150 focus:outline-none active:scale-95 disabled:opacity-50">
                        Simpan ke RAB <svg class="w-4 h-4 inline ml-1" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
    ...
    <div id="kategori-modal-overlay"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden">
            ...
            <div
                ...
                <button id="kategori-modal-close" type="button"
                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-white/10 hover:bg-white/20 transition-colors focus:outline-none">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M6 18L18 6M6 6l12 12" />
            ...
            <!-- Modal Footer -->
            <div
                class="flex items-center justify-between px-6 py-4 border-t border-table-border bg-white shrink-0 rounded-b-2xl shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                <p id="kategori-modal-info" class="text-xs text-table-subtle font-medium">0 kategori dipilih</p>
                <div class="flex items-center gap-2">
                    <button id="kategori-modal-cancel" type="button"
                        class="px-4 py-2 rounded-lg border border-table-border bg-white hover:bg-slate-50 text-table-body text-xs font-medium transition-all focus:outline-none active:scale-95">Batal</button>
                    <button id="kategori-modal-confirm" type="button"
                        class="px-5 py-2 rounded-lg bg-primary hover:bg-primary-hover text-white text-xs font-semibold tracking-wide shadow-sm transition-all duration-150 focus:outline-none active:scale-95">Tambahkan</button>
                </div>
            </div>
        </div>
    </div>
    ...
    ════════════════════════════════════════════════════════════════ -->
    <div id="import-prompt-modal-overlay"
        class="fixed inset-0 z-50 hidden items-center justify-center bg-black/60 backdrop-blur-sm p-4 opacity-0 transition-opacity duration-300">
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-sm flex flex-col overflow-hidden transform scale-95 transition-transform duration-300"
            id="import-prompt-modal-content">
            <!-- Modal Body -->
            <div class="px-6 py-8 text-center flex flex-col items-center">
                <!-- Icon container -->
                <div
                    class="w-16 h-16 bg-blue-50 text-primary rounded-full flex items-center justify-center mb-5 rotate-3 shadow-inner">
                ...
                <p class="text-[13px] text-slate-600 mb-1 leading-relaxed">Apakah Anda sudah memiliki file Excel untuk
                    di-import?</p>
                <p class="text-xs text-slate-400 mb-8 leading-relaxed px-2">Jika belum, silakan unduh template Excel
                    yang telah kami siapkan.</p>

                <div class="flex flex-col w-full gap-3">
                    <button type="button" id="import-prompt-modal-excel"
                        class="group relative w-full py-3 px-4 rounded-xl bg-primary hover:bg-primary-hover text-white text-sm font-bold tracking-wide shadow-md shadow-primary/20 transition-all duration-200 active:scale-[0.98] overflow-hidden flex items-center justify-center gap-2">
                        <div
                    ...
                    <button type="button" id="import-prompt-modal-template"
                        class="w-full py-3 px-4 rounded-xl border border-amber-300 bg-amber-50 hover:bg-amber-100 text-amber-700 text-sm font-bold tracking-wide transition-all duration-200 active:scale-[0.98] flex items-center justify-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                    ...
                    <button type="button" id="import-prompt-modal-cancel"
                        class="mt-4 text-[11px] font-semibold text-slate-400 hover:text-slate-600 transition-colors uppercase tracking-widest focus:outline-none">
                        Batal
                    </button>
                </div>
            </div>
    ...
    <div id="format-penomoran-modal-overlay"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col overflow-hidden">
            <div
                class="flex items-center justify-between px-6 py-4 border-b border-table-border bg-indigo-600 text-white rounded-t-2xl shrink-0">
                <div class="flex items-center gap-3">
                ...
                <button id="format-penomoran-modal-close" type="button"
                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-white/10 hover:bg-white/20 transition-colors focus:outline-none">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M6 18L18 6M6 6l12 12" />
            ...
            <div
                ...
                <button id="format-penomoran-modal-cancel" type="button"
                    class="px-4 py-2 rounded-lg border border-table-border bg-white hover:bg-slate-50 text-table-body text-xs font-medium transition-all focus:outline-none">Batal</button>
                <button id="format-penomoran-modal-save" type="button"
                    class="px-5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold tracking-wide shadow-sm transition-all focus:outline-none">Simpan
                    Format</button>
            </div>
        </div>
    </div>

                ...
                <div class="p-5 pl-6">
                    <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                        ...
                        <div class="shrink-0 flex items-center justify-end mt-2 sm:mt-0 gap-2">
                            ${req.status === 'pending' ? `
                                <button type="button" data-id="${req.id}" class="btn-delete-request px-3 py-2 text-xs font-bold text-rose-600 hover:text-white bg-white border border-rose-200 hover:bg-rose-600 rounded-lg transition-all focus:outline-none flex items-center gap-2 shadow-sm" title="Batalkan Permintaan">
                                    <i class="fas fa-trash-alt text-[10px]"></i>
                                </button>
                            ` : ''}
                            <button type="button" data-id="${req.id}" class="btn-detail-ajax px-4 py-2 text-xs font-bold text-slate-600 hover:text-blue-600 bg-white border border-slate-200 hover:border-blue-200 hover:bg-blue-50 rounded-lg transition-all focus:outline-none flex items-center gap-2 shadow-sm">
...
export function renderDetailModal(req, userRole) {
    ...
    if (!titleText || !modalBody || !actionsContainer) return;
        ...
        req.projects.forEach(proj => {
            ...
            ['Bahan', 'Alat'].forEach(cat => {
                if (groupedItems[cat].length > 0) {
                    ...
                    itemsHtml += `
                        <div class="mt-2 mb-1.5 flex items-center gap-2 px-1">
                    ...
                    groupedItems[cat].forEach(item => {
                        itemsHtml += `
                            <div class="flex items-center justify-between py-1.5 border-b border-slate-100 last:border-0 px-1 hover:bg-slate-50/50 transition-colors">
    ...
    let actionsHtml = `
        <button type="button" class="px-4 py-2 text-xs font-bold text-slate-500 hover:text-slate-700 bg-white border border-slate-200 rounded-lg focus:outline-none hover:bg-slate-50 transition-colors" data-hs-overlay="#modal-detail-permintaan">
            Tutup
        </button>
    `;
    ...
    projectRows.forEach((row, index) => {
        ...
        if (row.rapItems && row.rapItems.length > 0) {
            ...
            row.rapItems.forEach(item => {
                ...
                const optionStr = `<option value="${item.id_rap_detail_item}">${label}</option>`;

...
function _renderPreview(dataArray, tbody, countDisplay) {
    ...
    dataArray.forEach((item, index) => {
        ...
        const select    = `
            <select class="category-select w-full border border-table-border rounded bg-white text-xs px-2 py-1.5 focus:ring-1 focus:ring-primary outline-none transition-all" data-index="${index}">
        ...
        html += `
            <tr class="border-b border-table-border ${bgClass} hover:bg-slate-50 transition-colors">
                ...
                <td class="px-4 py-2 text-center">${isHeader ? '' : select}</td>
            </tr>`;
    });
...
export function initImport() {
    ...
    if (fileInput) {
        fileInput.addEventListener('change', async (e) => {
            ...
            try {
                ...
                _renderPreview(parsedData, tbody, countDisplay);
            } catch (err) {
                console.error('Gagal membaca Excel: ', err);
                tbody.innerHTML = `<tr><td colspan="8" class="text-center py-10 text-red-500 text-xs italic font-semibold">Gagal membaca file Excel.<br/><span class="font-normal text-[10px] text-red-400">${err && err.message ? err.message : String(err)}</span></td></tr>`;
                countDisplay.textContent = '0 baris terdeteksi';
            }
        });
    }
    ...
    if (modalConfirm) {
        modalConfirm.addEventListener('click', () => {
            if (parsedData.length === 0) { alert('Tidak ada data untuk diimpor.'); return; }

...
export async function openKategoriModal() {
    ...
    try {
        const list = await fetchKategoriMaster();
        ...
        list.forEach(cat => {
            const alreadyAdded = state.activeCategories.some(c => String(c.id) === String(cat.id));

            const li = document.createElement('li');
            li.innerHTML = `
                <label class="flex items-center gap-3 p-3 rounded-xl border ${alreadyAdded
        ...
        kategoriModalList.innerHTML = `
            <li class="p-4 text-center text-sm text-red-500">
...
export function updateModalInfo() {
    ...
    if (kategoriModalInfo) {
        kategoriModalInfo.textContent = `${total} kategori dipilih`;
    }
}
    ...
    catTr.querySelectorAll('.add-subitem-btn').forEach(btn => {
        btn.addEventListener('click', function (e) {
            ...
            try {
                ...
                const idProject = urlParams.get('id') || window.RAB_INIT?.idProject || window.RAB_INIT?.id || '';
                if (idProject) {
                    sessionStorage.setItem('current_id_project', idProject);
                }
            } catch (_) { }

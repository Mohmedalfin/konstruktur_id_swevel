...
export function renderSCurveChart() {
    ...
    // Cari gantt cell: td dengan colspan di baris manapun dalam tbody
    const ganttCell = tbody ? tbody.querySelector('td[colspan]') : null;

    if (!tbody || !ganttCell || !container) {
        hideSCurveChart();
        return;
    }

    // Hitung posisi relatif terhadap parent container (bukan viewport)
    const containerRect  = container.getBoundingClientRect();
    const tbodyRect      = tbody.getBoundingClientRect();
    const ganttCellRect  = ganttCell.getBoundingClientRect();

    const topRelative  = tbodyRect.top  - containerRect.top  + container.scrollTop;
    const leftRelative = ganttCellRect.left - containerRect.left + container.scrollLeft;
    const width        = ganttCellRect.width;
    const height       = tbodyRect.height;

    overlay.style.display       = 'block';
    overlay.style.position      = 'absolute';
    overlay.style.top           = `${topRelative}px`;
    overlay.style.left          = `${leftRelative}px`;
